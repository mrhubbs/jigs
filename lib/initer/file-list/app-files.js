...
import jigsConfig from '@Root/jigs.config.js'
...
export function createApp() {
  ...
  // make the config metadata available to all components
  Vue.use({
    install(v) {
      v.prototype.metadata = jigsConfig.metadata
      // TODO: we shouldn't do this based on NODE_ENV, but based on whether or
      // not the app will be served up via the dev server or written as static
      // files and served up via some other server
      v.prototype.metadata.baseurl = process.env.NODE_ENV === 'development' ? '' : jigsConfig.metadata.baseurl
    }
  })
