...
// exist.
export const objectToTree = (items, parentKey = 'parent') => {
  let nodes = toNodes(items)
  // Collect all the nodes with no parents into a root object.
  let [ root, rest ] = collectRoots(nodes, parentKey)
  ...
  Object.entries(rest).forEach(([ name, node]) => {
    ...
    if (root.children[parentName] !== undefined) {
      addChild(root.children[parentName], name, node)
    // is the parent in the rest?
    } else if (rest[parentName] !== undefined) {
      addChild(rest[parentName], name, node)
  ...
  const cycles = detectCycles(rest)
...
export const collectRoots = (nodes, parentKey) => {
...
export const makeNode = (data, parent = null, children = { }) => {
  return {
    data,
    parent,
    children,
    inCycle: null  // TODO: don't want to have to use this flag to support `detectCycles`
  }
}

// Adds a child to the given node.
export const addChild = (node, name, child) => {
  node.children[name] = child
  child.parent = node
}

// converts an object of objects to an object of nodes
export const toNodes = items =>
...
export const visitEveryChild = (tree, callback, level) => {
  if (level === undefined) {
    level = 0
  }
  ...
  Object.values(tree.children).forEach(node => {
    callback(node, level)
    visitEveryChild(node, callback, level + 1)
  })
}
...
export const detectCycles = nodes => {
...
export const detectCycle = (startingNode, curNode) => {
  // we haven't even started yet, kick off the recursive search...
  if (curNode === undefined) {
    let r = detectCycle(startingNode, startingNode.parent)
  ...
  let r = detectCycle(startingNode, curNode.parent)
