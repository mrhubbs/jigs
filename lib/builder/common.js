...
import { linterConfig, turnOffValidTemplateRoot } from './linter'

const linter = new CLIEngine(turnOffValidTemplateRoot(linterConfig))
...
const extractFrontMatter = curry(rawTemplate => {
  ...
  if (rawTemplate.startsWith('---\n')) {
    ...
    let frontMatter = yaml.safeLoad(match)  // could be undefined
...
export const loadResource = curry((rootPath, resPath) =>
  compose(
    bimap(
      err => `${err}, file "${path.relative(rootPath, resPath)}"`,
      identity
    ),
    map(
