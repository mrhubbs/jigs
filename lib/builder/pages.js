...
// ForgeConfig -> Either(Error String, Pages) -> String -> Either(Error String, Page)
// Reloads the given page and returns it
export const reloadPage = (config, pages, pagePath) => {
  let page = null

  // Get at the pages, if we can
  pages.map(ps => {
    // Create a shallow copy of the pages
    let newPages = {
      ...ps
    }
  ...
  return page === null ? new Error(`Could not load page at ${pagePath}`) : page
}

// Either(Error String, { Page }) -> String -> ()
// NOTE: this doesn't mutate the underlying data, but it does change the pages
// in the container.
export const removePage = (pages, name) => {
  let newPages = null
  ...
  pages.map(ps => {
    let newPages = { ...ps }
    // remove the page
    newPages[name] = undefined
  })

  return newPages === null ? pages : newPages
}
