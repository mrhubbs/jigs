...
const getWebpackConfig = (mode, forgeConfig) => {
  ...
  const defaultWebpackConfig = makeDefaultWebpackConfig(mode, forgeConfig)
...
export const build = forgeConfig => {
  return Async((reject, resolve) => {
    ...
    Webpack(
      getWebpackConfig('production', forgeConfig)
    ).run(
...
export const getMiddleware = forgeConfig => {
  const webpackConfig = getWebpackConfig('development', forgeConfig)
    ...
    WebpackMiddleware(Webpack(webpackConfig), {
      ...
      reporter: (middlewareOptions, options) => {
        ...
        if (state) {
          ...
          if (stats.hasErrors()) {
            logging.logFailure('Failed to compile')
          } else if (stats.hasWarnings()) {
            logging.logInfo('Compiled with warnings')
          } else {
            logging.logDetail('Compiled successfully')
          }
        } else {
          logging.logDetail('Compiling...')
        }
      },
