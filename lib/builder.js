...
const makeNonCssForge = (config, mode) => {
  let b = makeBaseForge(config)
    ...
    // If the project has a webpack config file, we'll use webpack to process
    // the JavaScript. If it doesn't, any .js files will just be copied over to
    // the build directory.
    const wpc = path.join(process.cwd(), 'webpack.config.js')
    if (jetpack.exists(wpc)) {
      logInfo('Using Webpack to process .js files.')

      const webpack = require('metalsmith-webpack');
      const webpackConfig = require(wpc);

      b.use(webpack(
        webpackConfig(config)
      ));

      // TODO: filter out source .js files
    }
