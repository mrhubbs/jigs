...
function makeRenderer(jigsConfig, serverBundlePath, clientManifestPath) {
  ...
  // TODO: this'll catch the error, but why is it happening?
  try {
    serverBundle = JSON.parse(jetpack.read(serverBundlePath))
  } catch (err) {
    throw new Error(`Error trying to read server bundle: ${err}`)
  }
  try {
    clientManifest = JSON.parse(jetpack.read(clientManifestPath))
  } catch (err) {
    throw new Error(`Error trying to read client manifest: ${err}`)
  }
